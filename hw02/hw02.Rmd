...
ggplot(monona, aes(x = duration)) +
  ...
  ylab("Total") +
  ggtitle("Lake Monona Days Closed with Ice, 1855-2022")
```
...
It creates a line of best fit, without the shaded confidence interval
...
The curve is decreasing over time
...
Lake Monona was closed with ice for around 117 days in a typical year near 1875
...
Lake Monona was closed with ice for around 90 days in a typical year near 1875
...
The approximate median number of days Lake Monona has been closed with ice is 103 days



  5. Write code to create a bar graph which displays the number of winters when the first freeze occurred in each half-month period of time as recorded in the variable `ff_cat`. Choose your own colors if you do not like the default values. Make sure that your plot:
...
ggplot(monona, aes(x = ff_cat)) +
  geom_bar() +
  xlab("Half-month period first freeze") + 
  ggtitle("Lake Monona First Freeze, 1855-2022")
```
...
Because it's easier to compare values when the bars are vertical, geom_bar() counts the number of occurences, unlike geom_col() which doesn't
...
ggplot(monona, aes(x = ff_x, y = duration, color = period50)) +
  ...
  ggtitle("Lake Monona Freeze Durations over Time, 1855-2022")
```
...
My prediction of the total duration would be higher if the winter was in the 1870s, versus the present which is lower
...
Yes, color and faceting makes it easier to compare and see differences between the relationship
