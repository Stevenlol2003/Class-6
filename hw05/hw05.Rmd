...
> `num` is number of times where females have a higher percent than males for a zip code

> `n` is number of none NA rows

> `p_1` is the proportion of zip codes where females have a higher percent than males for a zip code

> `p_2` is the proportion of zip codes where females have the same perfecent as males for a zip code
...
ggplot(education_orig, aes(x = pct_f_bach, y = pct_m_bach)) +
  ...
  xlab("Female Degree Percentage") +
  ylab("Male Degree Percentage") +
  ggtitle("Female vs Male Degree Percentage Among the Various Zip Codes in Wisconsin")
...
> The pattern in the plot shows the females have a higher perfectage of adults with bachelor's degrees than males in most zip codes in Wisconsin

  2c. What do you think might be true about the zip codes with strong outliers, for example, where percentages are 0% or 100%?

> I think those zip code locations are missing data and should not be included in these plots
...
#  filter(pct_m_bach != "NA" & pct_f_bach != "NA") %>%
  pivot_longer(cols = 2:3, names_to = "sex",values_to = "pct_bachelors", values_drop_na = TRUE)

education$zip = as.character(education$zip)

education$sex[education$sex == "pct_m_bach"] = "male"
education$sex[education$sex == "pct_f_bach"] = "female"
...
obesity = read_csv("../../data/obesity.csv") %>%
  mutate(obese_pop = pop * (obese/n), non_obese_pop = pop * (1 - obese/n))
...
ggplot(adult_pop_sum, aes(x = total_pop, y = sex)) +
  ...
  xlab("Population") +
  ylab("Gender") +
  ggtitle("Male and Female Population by Zip Code")
```
...
```{r}
table5a = merge(education, adult_pop_sum)
...
> There are 2 cases of 100% bachelors which seems weird, and one of them is population of 2961 with 100% bachelors, compared to the other 28 cases of 0% bachelors which is just missing data
...
table6a = table5a %>% 
  filter(sex == "male") %>% 
  group_by(sex) %>% 
  mutate(degree_pop = pct_bachelors * total_pop / 100) %>% 
  summarise(pct = sum(degree_pop) / sum(total_pop) * 100)
...
> The percentages of women and men ages 25 and older in Wisconsin with a bachelors degree are 27.4% for male, 29.8% for female
...
adult_obesity_sum = merge(obesity, correct_zip_codes) %>% 
  filter(count == 8) %>% 
  filter(!age == "05-17") %>% 
  group_by(zip, sex) %>% 
  summarize(total_obese = sum(obese_pop), total_pop = sum(pop))
  
dim(adult_obesity_sum)
...
```{r}
table8 = merge(adult_obesity_sum, education, by = c("zip", "sex")) %>% 
  mutate(pct_obese = total_obese / total_pop * 100)

ggplot(table8, aes(x = pct_bachelors, y = pct_obese, color = sex)) +
  geom_point() +
  geom_smooth(method=lm, se=FALSE) +
  xlab("Percent Bachelors") +
  ylab("Percent Obese") +
  ggtitle("Percent Bachelors vs Percent Obese by Sex")
```

> As education percentage incrases, obesity rate decreases for men and women
